...
let output;
try {
  output = execSync('npx eslint "src/**/*.{ts,tsx,js,jsx}" --format json', { encoding: 'utf-8', stdio: 'pipe', maxBuffer: 10 * 1024 * 1024 });
} catch (e) {
...
for (const result of results) {
  ...
  for (const msg of messages) {
    if (msg.ruleId === '@typescript-eslint/no-unused-vars' || msg.ruleId === '@typescript-eslint/no-explicit-any') {
      ...
      if (prevLine.includes('eslint-disable-next-line') && prevLine.includes(msg.ruleId)) {
        continue;
      }
