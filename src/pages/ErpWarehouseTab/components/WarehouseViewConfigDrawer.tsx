...
import {
  WAREHOUSE_COLUMN_GROUPS,
  WAREHOUSE_COLUMN_VIEW_PRESETS,
  DEFAULT_WAREHOUSE_COLUMN_VISIBILITY,
} from "../utils";

export interface WarehouseViewConfigDrawerProps {
  open: boolean;
  onClose: () => void;
...
export function WarehouseViewConfigDrawer({
  open,
  onClose,
  preset,
  ...
  onSave,
  onResetDefault,
}: WarehouseViewConfigDrawerProps) {
  ...
  useEffect(() => {
    if (open) {
      if (preset) {
        ...
        WAREHOUSE_COLUMN_GROUPS.forEach((group) => {
          group.columns.forEach((col) => {
            if (
              ...
              initialVis[col.key] =
                DEFAULT_WAREHOUSE_COLUMN_VISIBILITY[col.key] !== false;
            }
          });
        });
        ...
        <div className="space-y-3 pb-4">
            ...
            >
              <input
                ...
                placeholder={t("viewConfigNamePlaceholder", {
                  defaultValue: "VD: Tra cứu nhanh chứng từ",
                })}
                value={name}
          ...
          >
            <div className="space-y-3 pt-1">
              {WAREHOUSE_COLUMN_GROUPS.map((group) => (
                <DrawerSection
                  key={group.groupKey}
