...
import { updateProfileApi, getEmployeeApi, type Employee, type UpdateProfileRequest } from "@/modules/auth/api/auth";
import { useAuthStore } from "@/modules/auth/domain/authStore";
import { getRolesApi, getEmployeePermissionsApi, saveEmployeePermissionsApi } from "@/modules/system/api/rbacApi";
...
import { buildForm, emptyForm, getDirectusId, IconPlus, IconRefresh, STATUS_FILTER_FETCH_LIMIT, type EmpForm } from "@/modules/hr/components/NhanSu/shared";
...
export function NhanSu() {
  ...
  const policyEditor = usePermissionsEditor({ getApi: (id) => getEmployeePermissionsApi(id), saveApi: (id, dto) => saveEmployeePermissionsApi(id, dto) });
  ...
  const statusLabel = useMemo(() => ({ active: t("nhansu.status.active"), inactive: t("nhansu.status.inactive"), resigned: t("nhansu.status.resigned") }), [t]);
  const isDirty = !!form.full_name.trim() || !!form.email.trim();

  useEffect(() => {
    Promise.all([getDepartmentsApi(), getPositionsApi(), getRolesApi({ pageSize: 200 })])
      .then(([d, p, r]) => { setDepts(d); setPositions(p); setAllRoles(r.items); })
      .catch(() => {});
  }, []);

  useEffect(() => { loadData(page, pageSize, search, statusFilter); }, [page, pageSize, search, statusFilter]);

  async function loadData(pg: number, ps: number, q: string, status: string) {
    setLoading(true);
    setFetchError(null);
    try {
      const result = await getEmployeesPagedApi({ page: status ? 1 : pg, pageSize: status ? STATUS_FILTER_FETCH_LIMIT : ps, search: q || undefined });
      if (status) {
        const filtered = result.items.filter((emp) => emp.employment_status === status);
        const start = (pg - 1) * ps;
  ...
  function handleSearchInput(value: string) {
    ...
    if (value === "") { setSearch(""); setPage(1); return; }
  ...
  const openNew = () => { setEditing(null); setForm(emptyForm); setSaveError(null); setDrawerOpen(true); };
  const openEdit = (item: Employee) => { setEditing(item); setForm(buildForm(item)); setSaveError(null); setDrawerOpen(true); };
  const closeDrawer = () => { setDrawerOpen(false); setEditing(null); setSaveError(null); };
  const openPolicyMatrix = (emp: Employee) => { setPolicyTarget(emp); policyEditor.reset(); setPolicyDrawerOpen(true); policyEditor.loadPermissions(emp.id); };

  async function handleSavePolicyPermissions() {
    try {
      await policyEditor.save();
      showToast({ title: t("nhansu.toast.permissionsSaved"), variant: "success" });
      setPolicyDrawerOpen(false);
      policyEditor.reset();
    } catch (e) {
      showToast({ title: t("nhansu.toast.permissionsFailed"), description: e instanceof Error ? e.message : "", variant: "destructive" });
    }
  }

  async function handleSave() {
    if (!form.full_name.trim()) return setSaveError(t("nhansu.errors.nameRequired"));
    if (!form.email.trim()) return setSaveError(t("nhansu.errors.emailRequired"));
    setSaving(true);
    setSaveError(null);
    try {
  ...
  async function handleImpersonate() {
    ...
    try {
      ...
      if (!targetId) targetId = getDirectusId(await getEmployeeApi(impersonateTarget.id));
      if (!targetId) {
      ...
      showToast({ title: t("nhansu.toast.impersonateStarted"), description: impersonateTarget.full_name, variant: "success" });
    } catch (e) {
      const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message;
      showToast({ title: t("nhansu.toast.impersonateFailed"), description: msg ?? "", variant: "destructive" });
    } finally {
...
function HeaderActions({ onRefresh, onNew, t }: { onRefresh: () => void; onNew: () => void; t: ReturnType<typeof useT> }) {
  return <><button onClick={onRefresh} title={t("nhansu.actions.refresh")} className="w-8 h-8 rounded-lg border border-border bg-surface flex items-center justify-center text-[color:var(--muted-fg)] hover:bg-surface-hover cursor-pointer"><IconRefresh /></button><button onClick={onNew} className="px-[14px] py-[7px] rounded-lg border border-primary bg-primary text-primary-fg text-xs font-medium cursor-pointer flex items-center gap-[6px] hover:opacity-90 whitespace-nowrap"><IconPlus /> {t("nhansu.actions.add")}</button></>;
}
