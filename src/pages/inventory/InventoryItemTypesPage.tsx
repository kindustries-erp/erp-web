...
export function InventoryItemTypesPage() {
  ...
  const currentQuery = useInventoryMasterListQuery(itemTypeParams);
  ...
  return (
    <>
      <SpreadsheetPageTemplate<InventoryMasterOption>
        title={t("nav.items.erpInventoryItemTypes") || "Loại hàng hóa"}
        desc={t("inventoryMasters.tabs.itemTypeDesc")}
        icon={<Boxes className="h-5 w-5" />}
        tableId="inventory-item-types-table"
        items={currentItems}
        columns={columns}
        ...
        createActions={[
          {
            groupLabel: "Loại hàng hóa",
            items: [
              {
                label: t("inventoryMasters.actions.createItemType"),
