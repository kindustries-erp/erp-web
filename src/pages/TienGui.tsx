...
import { getChartOfAccountsApi, getCompanyBankAccountsApi, type ChartOfAccount, type CompanyBankAccount } from "@/modules/accounting/api/catalogApi";
...
import { useVoucherAttachments, useVoucherList } from "@/modules/finance/hooks/useVoucherList";
import { useAmountRangeFilter, useSearchFilter } from "@/shared/hooks/useFilterState";
...
export function TienGui() {
  ...
  const { period, dateFrom, dateTo, channelFilter: bankFilter, hasActiveFilter: hasActivePeriodFilter, handlePeriodChange: handlePeriodFilterChange, handleDateFrom: handlePeriodDateFrom, handleDateTo: handlePeriodDateTo, handleChannelFilter: handleBankFilterChange, resetPeriod } = periodFilter;
  const dashboard = useVoucherDashboard();
  const { summary, summaryLoading, openingBal, openingLoading, chartData, chartLabels, chartYMax, chartUnit, receiptDonutItems, paymentDonutItems, donutLoading, loadSummary, loadOpeningBalanceAndChart, loadDonutData } = dashboard;
  const voucherList = useVoucherList();
  const { vouchers, loading, fetchError, page, setPage, pageSize, total, totalPages, sortCol, handlePageSize, handleSort, loadVouchers } = voucherList;
  const { voucherAttachments, loadVoucherAttachments } = useVoucherAttachments();
  const { searchInput, search, handleSearchInput } = useSearchFilter();
  const amountFilter = useAmountRangeFilter();
  const { amountMinInput, amountMaxInput, amountMin, amountMax, handleAmountRangeInput, resetAmounts } = amountFilter;

  const bankDashboardParams = useMemo(() => ({ voucherChannel: "BANK" as const, channelParam: "company_bank_account_id" as const, channelFilter: bankFilter, receiptType: "BANK_RECEIPT" as const, paymentType: "BANK_PAYMENT" as const }), [bankFilter]);

  useEffect(() => {
    Promise.all([getCompanyBankAccountsApi(), getChartOfAccountsApi(), getPaymentVoucherLookupBusinessPartnersApi(), getPaymentVoucherLookupEmployeesApi()])
      .then(([banks, coa, bps, emps]) => { setCompanyBankAccounts(banks ?? []); setCoaItems(coa ?? []); setPartners(bps ?? []); setEmployees(emps ?? []); })
      .catch(() => {})
  ...
  useEffect(() => { loadSummary(dateFrom, dateTo, bankDashboardParams); }, [dateFrom, dateTo, bankDashboardParams, loadSummary]);
  useEffect(() => { loadOpeningBalanceAndChart(dateFrom, dateTo || TODAY, bankDashboardParams); }, [dateFrom, dateTo, bankDashboardParams, loadOpeningBalanceAndChart]);
  useEffect(() => { if (catalogLoaded) loadDonutData(bankFilter, coaItems, bankDashboardParams, dateFrom, dateTo); }, [catalogLoaded, bankFilter, coaItems, bankDashboardParams, dateFrom, dateTo, loadDonutData]);
  useEffect(() => {
    loadVouchers({ page, pageSize, search, statusFilter, channelFilter: bankFilter, channelParam: "company_bank_account_id", voucherChannel: "BANK", sortCol, dateFrom, dateTo, amountMin, amountMax, counterpartySourceFilter }).then((items) => { if (items) loadVoucherAttachments(items); });
  }, [page, pageSize, search, statusFilter, bankFilter, sortCol, dateFrom, dateTo, amountMin, amountMax, counterpartySourceFilter, loadVouchers, loadVoucherAttachments]);

  const handlePeriodChange = useCallback((value: string) => { handlePeriodFilterChange(value); setPage(1); }, [handlePeriodFilterChange, setPage]);
  const handleDateFrom = useCallback((value: string) => { handlePeriodDateFrom(value); setPage(1); }, [handlePeriodDateFrom, setPage]);
  const handleDateTo = useCallback((value: string) => { handlePeriodDateTo(value); setPage(1); }, [handlePeriodDateTo, setPage]);
  const handleBankFilter = useCallback((value: string) => { handleBankFilterChange(value); setPage(1); }, [handleBankFilterChange, setPage]);
  const handleReset = useCallback(() => { resetPeriod(); resetAmounts(); setPage(1); }, [resetAmounts, resetPeriod, setPage]);
  const handleStatusFilter = useCallback((value: string) => { setStatusFilter(value as VoucherStatus | ""); setPage(1); }, [setPage]);

  const hasActiveFilter = hasActivePeriodFilter || !!amountMin || !!amountMax;
  const fmtAmount = useCallback((n: number) => { const abs = Math.abs(n); if (abs >= 1_000_000_000) return `₫ ${(n / 1_000_000_000).toFixed(1)}B`; if (abs >= 1_000_000) return `₫ ${(n / 1_000_000).toFixed(0)}M`; return `₫ ${n.toLocaleString("vi-VN")}`; }, []);
  const isDirty = !!form.voucher_no.trim() || !!form.amount;
  const optionSets = useMemo(() => buildOptionSets(coaItems, partners, employees, companyBankAccounts, partnerBankAccounts, form.voucher_type), [coaItems, partners, employees, companyBankAccounts, partnerBankAccounts, form.voucher_type]);
...
function buildOptionSets(coaItems: ChartOfAccount[], partners: BusinessPartner[], employees: Employee[], companyBankAccounts: CompanyBankAccount[], partnerBankAccounts: any[], voucherType: string) {
  ...
  const bankAccountOpts = coaItems.filter((c) => c.account_code.startsWith("112")).map((c) => ({ value: c.id, label: `${c.account_code} — ${c.account_name}` }));
  return {
    coaOpts,
    ...
    partnerBankOpts: partnerBankAccounts.map((b) => ({ value: b.id, label: `${b.bank_name} — ${b.account_number}` })),
  };
}
