...
export const useSettingsStore = create<SettingsState>((set) => ({
  ...
  setDefault: (type, index) => {
    if (type === "quy") {
      set((s) => ({
        quyData: s.quyData.map((q, i) => ({ ...q, isDefault: i === index })),
      }));
    } else {
      set((s) => ({
        nhData: s.nhData.map((n, i) => ({ ...n, isDefault: i === index })),
      }));
    }
  },
  ...
  toggleActive: (type, index) => {
    if (type === "quy") {
      set((s) => ({
        quyData: s.quyData.map((q, i) =>
          i === index ? { ...q, active: !q.active } : q,
        ),
      }));
    } else if (type === "nh") {
      set((s) => ({
        nhData: s.nhData.map((n, i) =>
          i === index ? { ...n, active: !n.active } : n,
        ),
      }));
    } else {
      set((s) => ({
        tkData: s.tkData.map((t, i) =>
