...
function makeTx(n: number, pfx: string, quys: string[]): Transaction[] {
  ...
  const dt = ['Cty TNHH Công Nghệ ABC', 'Nguyễn Văn A', 'Khối Sản Xuất', 'THANH CH Công Trăng', 'Cty CP Phát Triển XYZ']
  const am = [50000000, 20000000, 1500000, 5000000, 12000000, 8000000, 30000000, 3000000, 45000000, 7500000]
  ...
  return Array.from({ length: n }, (_, i) => {
    ...
    return {
      ...
      code: pfx + (t === 'thu' ? 'PT' : 'PC') + '0' + String(i + 1).padStart(3, '0'),
      type: t,
      loai: isNH
        ? (t === 'thu' ? lT[i % 3] : lC[i % 3])
        : (t === 'thu' ? tmT[i % 3] : tmC[i % 3]),
      doituong: dt[i % 5],
      mst: i % 2 === 0 ? '0101234567' : '',
      diachi: i % 2 === 0 ? 'Số 123 Nguyễn Trãi, Thanh Xuân, Hà Nội' : 'Hà Nội',
      dienGiai: isNH
        ? (t === 'thu' ? 'UNT thu tiền KH HĐ-00' + i : 'UNC thanh toán đề nghị ' + i)
        : (t === 'thu' ? 'Thu tiền KH HĐ-00' + i : 'Chi thanh toán đề nghị ' + i),
      amount: am[i % 10],
      tknNo: t === 'thu' ? (isNH ? '1121 - NH VCB' : '1111 - Tiền mặt') : '331 - Phải trả NCC',
      tknCo: t === 'thu' ? '1311 - Phải thu KH' : (isNH ? '1121 - NH VCB' : '1111 - Tiền mặt'),
