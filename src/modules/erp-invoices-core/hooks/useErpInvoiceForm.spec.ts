...
describe("useErpInvoiceForm", () => {
  ...
  it("should strip pendingDeletedPdfs and pendingAddedPdfs from payload when saving", async () => {
    ...
    vi.mocked(erpInvoicesCoreApi.update).mockResolvedValue({
      id: "inv-1",
      invoiceNo: "INV-1",
    } as any);
    vi.mocked(erpInvoicesCoreApi.deletePdf).mockResolvedValue({
      success: true,
    } as any);
    vi.mocked(erpInvoicesCoreApi.get).mockResolvedValue({
      id: "inv-1",
      invoiceNo: "INV-1",
      items: [],
    } as any);
    ...
    await act(async () => {
      await result.current.openInternal({
        id: "inv-1",
        invoiceNo: "INV-1",
        items: [],
      } as any);
    ...
    expect(erpInvoicesCoreApi.deletePdf).toHaveBeenCalledWith(
      "inv-1",
      "delete-me.pdf",
    );
  });
