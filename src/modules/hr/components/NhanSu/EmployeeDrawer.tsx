...
export function EmployeeDrawer(props: EmployeeDrawerProps) {
  ...
  return (
    <DrawerModal
      ...
      icon={<IconUser />}
      title={editing ? t("nhansu.drawer.editTitle") : t("nhansu.drawer.createTitle")}
      ...
      actions={[
        ...
        { label: editing ? t("nhansu.actions.saveChanges") : t("common.addNew"), primary: true, loading: saving, disabled: saving, onClick: onSave },
      ]}
    >
      <DrawerSection title={t("nhansu.drawer.personalInfo")}>
        <TextField label={t("nhansu.fields.employeeCode")} value={form.employee_code} onChange={(v) => setField("employee_code", v)} placeholder="VD: NV001" />
        <TextField label={t("nhansu.fields.fullName")} value={form.full_name} onChange={(v) => setField("full_name", v)} placeholder={t("nhansu.placeholders.fullName")} required />
        <TextField label={t("nhansu.fields.email")} type="email" value={form.email} onChange={(v) => setField("email", v)} placeholder="example@company.com" required />
        <TextField label={t("nhansu.fields.phone")} type="tel" value={form.phone} onChange={(v) => setField("phone", v)} placeholder="0912 345 678" />
      </DrawerSection>
      ...
      <DrawerSection title={t("nhansu.drawer.employment")}>
        <DrawerField label={t("nhansu.fields.status")}>
          <Combobox options={statusOptions} value={form.employment_status} onChange={(v) => setField("employment_status", v || "active")} allowClear={false} />
        </DrawerField>
        <TextField label={t("nhansu.fields.hireDate")} type="date" value={form.hire_date} onChange={(v) => setField("hire_date", v)} />
        <DrawerField label={t("nhansu.fields.accountActive")}>
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-xs text-foreground">{t("nhansu.fields.activeAccount")}</span>
          </label>
        </DrawerField>
      </DrawerSection>
      ...
      <DrawerSection title={t("nhansu.drawer.notes")}>
        <DrawerField label={t("nhansu.fields.internalNotes")}>
          <textarea className={cn(inputCls, "min-h-[70px] resize-none")} value={form.notes} onChange={(e) => setField("notes", e.target.value)} placeholder={t("nhansu.placeholders.notes")} rows={3} />
        </DrawerField>
      </DrawerSection>

      {editing && <PermissionsSection editing={editing} allRoles={allRoles} roleId={form.role_id} setRoleId={(v) => setField("role_id", v)} onOpenPolicyMatrix={onOpenPolicyMatrix} t={t} />}
...
function TextField({ label, value, onChange, placeholder, type = "text", required }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string; required?: boolean }) {
  return <DrawerField label={label} required={required}><input type={type} className={inputCls} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} /></DrawerField>;
}

function PermissionsSection({ editing, allRoles, roleId, setRoleId, onOpenPolicyMatrix, t }: { editing: Employee; allRoles: Role[]; roleId: string; setRoleId: (value: string) => void; onOpenPolicyMatrix: (emp: Employee) => void; t: T }) {
  return (
    <DrawerSection title={t("nhansu.drawer.permissions")}>
      <DrawerField label={t("nhansu.fields.baseRole")}>
        <Combobox options={allRoles.map((r) => ({ value: r.id, label: r.name }))} value={roleId} onChange={setRoleId} placeholder={t("nhansu.placeholders.noRole")} />
      </DrawerField>
      <DrawerField label={t("nhansu.fields.customPolicy")}>
        <button type="button" onClick={() => onOpenPolicyMatrix(editing)} className="w-full flex items-center justify-between px-3 py-[7px] text-xs rounded-lg border border-[color:var(--border)] bg-[color:var(--muted)] text-[color:var(--muted-fg)] hover:bg-surface-hover hover:text-foreground transition-colors">
