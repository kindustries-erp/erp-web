...
import {
  BANK_STATEMENT_COLUMN_GROUPS,
  BANK_STATEMENT_COLUMN_VIEW_PRESETS,
  DEFAULT_BANK_COLUMN_VISIBILITY,
} from "../utils";

export interface BankStatementViewConfigDrawerProps {
  open: boolean;
  onClose: () => void;
...
export function BankStatementViewConfigDrawer({
  open,
  onClose,
  preset,
  ...
  onSave,
  onResetDefault,
}: BankStatementViewConfigDrawerProps) {
  ...
  const getDefaultPresetLabel = (p?: TableViewPreset | null) => {
    ...
    if (p.key === "overview" && (!p.label || p.label === "Tổng quan")) {
      return t("viewModeOverview", { defaultValue: "Tổng quan" });
    }
    if (
      p.key === "audit" &&
      (!p.label || p.label === "Kiểm toán / Đối soát" || p.label === "Đối soát")
    ) {
      return t("viewModeAudit", { defaultValue: "Kiểm toán / Đối soát" });
    }
    return p.label;
  };
  ...
  useEffect(() => {
    if (open) {
      if (preset) {
        ...
        BANK_STATEMENT_COLUMN_GROUPS.forEach((group) => {
          group.columns.forEach((col) => {
            if (
              ...
              initialVis[col.key] =
                DEFAULT_BANK_COLUMN_VISIBILITY[col.key] !== false;
            }
          });
        });
  ...
  const handleSave = () => {
    if (!name.trim()) {
      toast.error(
        t("viewConfigNameRequired", {
          defaultValue: "Vui lòng nhập tên view",
        }),
      );
      return;
    }
    ...
    if (!hasAnySelected) {
      toast.error(
        t("viewConfigColumnsRequired", {
          defaultValue: "Vui lòng chọn ít nhất một cột",
        }),
      );
      return;
    }
  ...
  const drawerTitle = preset
    ? isDefaultPreset
      ? t("viewConfigEditDefaultTitle", {
          defaultValue: `Chỉnh sửa chế độ xem: ${getDefaultPresetLabel(preset)}`,
          name: getDefaultPresetLabel(preset),
        })
      : t("viewConfigEditTitle", { defaultValue: "Chỉnh sửa view" })
    : t("viewConfigTitle", { defaultValue: "Tạo view mới" });

  const actions = [
    {
      label: t("viewConfigSave", { defaultValue: "Lưu view" }),
      onClick: handleSave,
      primary: true,
    },
    ...(isDefaultPreset && onResetDefault
      ? [
          {
            label: t("viewModeReset", { defaultValue: "Khôi phục mặc định" }),
      ...
      : []),
    {
      label: t("viewConfigCancel", { defaultValue: "Hủy" }),
      onClick: onClose,
    ...
    <StandardFormDrawer
      ...
      leftPanel={
        <div className="space-y-3 pb-4">
          ...
          <DrawerSection
            title={t("viewConfigNameLabel", { defaultValue: "Tên view" })}
            collapsible
            defaultCollapsed={false}
          >
            <DrawerField
              label={t("viewConfigNameLabel", { defaultValue: "Tên view" })}
              required
            >
              <input
                type="text"
                className={inputCls}
                placeholder={t("viewConfigNamePlaceholder", {
                  defaultValue: "VD: Báo cáo đối soát ngân hàng",
                })}
                value={name}
          ...
          <DrawerSection
            title={t("viewConfigColumnsLabel", {
              defaultValue: "Cột hiển thị",
            })}
            collapsible
            defaultCollapsed={false}
            titleExtra={
              ...
              >
                ...
                >
                  {t("viewConfigSelectAll", { defaultValue: "Chọn tất cả" })}
                </button>
          ...
          >
            <div className="space-y-3 pt-1">
              {BANK_STATEMENT_COLUMN_GROUPS.map((group) => (
                <DrawerSection
                  key={group.groupKey}
