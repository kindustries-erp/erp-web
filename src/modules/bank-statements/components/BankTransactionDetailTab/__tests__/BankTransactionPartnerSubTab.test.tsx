...
describe("BankTransactionPartnerSubTab", () => {
  ...
  it("renders 1-based index STT starting from 1 and not 0", async () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <BankTransactionPartnerSubTab transaction={mockTransaction} />
      </QueryClientProvider>,
    );
  ...
  it("renders empty state when transaction has no partner data", async () => {
    const mockTransaction = {
      id: "root-2",
    ...
    expect(screen.getByText("Không có thông tin đối tác")).toBeTruthy();
  });
});
