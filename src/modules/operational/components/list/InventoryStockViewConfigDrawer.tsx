...
} from "./utils/stockViewPresets";

export interface InventoryStockViewConfigDrawerProps {
  open: boolean;
  onClose: () => void;
...
export function InventoryStockViewConfigDrawer({
  open,
  onClose,
  preset,
  ...
  onSave,
  onResetDefault,
}: InventoryStockViewConfigDrawerProps) {
  ...
  useEffect(() => {
    if (open) {
      if (preset) {
        ...
        STOCK_COLUMN_GROUPS.forEach((group) => {
          group.columns.forEach((col) => {
            if (
              ...
              initialVis[col.key] =
                DEFAULT_STOCK_COLUMN_VISIBILITY[col.key] !== false;
            }
          });
        });
  ...
  const handleSave = () => {
    if (!name.trim()) {
      toast.error(
        t("viewConfigNameRequired", { defaultValue: "Vui lòng nhập tên view" }),
      );
      return;
    }
            ...
            >
              <input
                ...
                placeholder={t("viewConfigNamePlaceholder", {
                  defaultValue: "VD: Tra cứu nhanh tồn kho",
                })}
                value={name}
