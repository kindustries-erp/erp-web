...
interface BudgetRecurringConfirmModalProps {
  open: boolean;
  onClose: () => void;
...
export function BudgetRecurringConfirmModal({
  open,
  onClose,
  onConfirm,
  loading = false,
  ...
  amount,
  title,
}: BudgetRecurringConfirmModalProps) {
      ...
      <DialogContent className="max-w-[460px] p-6 bg-card border border-border shadow-xl rounded-xl">
        <DialogHeader className="text-left space-y-2 mb-2">
          <div className="flex items-center gap-2 text-primary font-semibold text-base">
            ...
            <DialogTitle className="text-sm font-bold text-foreground">
              {t("recurringModal.title", "Áp dụng thay đổi định kỳ")}
            </DialogTitle>
          </div>
          <DialogDescription className="text-xs text-muted-foreground leading-relaxed">
            {t(
              "recurringModal.desc",
              "Khoản chi phí này thuộc chuỗi định kỳ lặp lại hàng tháng của doanh nghiệp. Vui lòng chọn phạm vi áp dụng thay đổi:",
            )}
          </DialogDescription>
        </DialogHeader>
          ...
          >
            ...
            <div className="flex flex-col gap-0.5 text-xs flex-1">
              <div className="flex items-center gap-1.5 font-semibold text-foreground">
                ...
                <span>
                  {t(
                    "recurringModal.scopeThisAndFuture",
              ...
              <p className="text-[11px] text-muted-foreground leading-normal">
                {t(
                  "recurringModal.scopeThisAndFutureDesc",
        ...
        <div className="p-3 bg-muted/40 rounded-lg border border-border/60 text-xs flex flex-col gap-1.5">
          <div className="flex justify-between items-center text-muted-foreground">
            <span>{t("recurringModal.category", "Khoản chi")}:</span>
            <span className="font-semibold text-foreground truncate max-w-[240px]">
              {title || "—"}
            </span>
          </div>
          <div className="flex justify-between items-center text-muted-foreground">
            <span>{t("recurringModal.newAmount", "Số tiền áp dụng")}:</span>
        ...
        <DialogFooter className="flex-row justify-end gap-2 mt-4 sm:space-x-0">
          ...
          >
            ...
            <span>
              {loading
                ? t("saving", "Đang xử lý...")
                : t("recurringModal.confirmApply", "Xác nhận áp dụng")}
            </span>
          </Button>
        </DialogFooter>
      </DialogContent>
