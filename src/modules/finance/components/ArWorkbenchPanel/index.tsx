...
export function ArWorkbenchPanel() {
  ...
  const openDrawer = (mode: "INVOICE" | "OTHER" = "INVOICE") => {
    ...
    if (mode === "OTHER") { next.document_no = `THU-KHAC-${Date.now().toString(36).toUpperCase()}`; next.description = "Thu chứng từ khác"; next.lines = [{ line_no: 1, description: "Thu chứng từ khác", quantity: 1, unit_price: 0, tax_rate: 0 }]; }
    setForm(next);
    setDrawerOpen(true);
  ...
  const saveDocument = () => {
    setSaving(true);
    setSaveError(null);
    const totalAmount = form.lines.reduce((sum, line) => sum + line.quantity * line.unit_price * (1 + (line.tax_rate ?? 0) / 100), 0);
    const request = createMode === "INVOICE"
      ? createArSalesInvoiceApi({
          ...form,
      ...
      : createArDocumentApi({
          ...
          metadata: { ...(form.metadata ?? {}), entry_type: "OTHER_RECEIVABLE" },
        });
    request
      .then(() => { setDrawerOpen(false); setForm(emptySalesInvoiceForm()); load(); })
      .catch((err) => setSaveError(extractApiError(err, createMode === "INVOICE" ? "Không tạo được hóa đơn" : "Không tạo được thu khác")))
...
function InvoiceKpis({ summary, supported, coverageTotal, foundation }: { summary: ArSummary | null; supported: number; coverageTotal: number; foundation: number }) {
  return <div className="grid gap-3 md:grid-cols-4">
    <Kpi label="Open amount" value={`${money(summary?.totals.open_amount)} VND`} />
    <Kpi label="Overdue" value={`${money(summary?.totals.overdue_amount)} VND`} warn />
    <Kpi label="Documents" value={summary?.totals.count ?? 0} />
    <Kpi label="Use case coverage" value={`${supported}/${coverageTotal}`} sub={`${foundation} use case cần hardening phase sau`} />
  </div>;
}

function Kpi({ label, value, sub, warn }: { label: string; value: string | number; sub?: string; warn?: boolean }) {
  return <div className="rounded-xl border border-[color:var(--border)] bg-[color:var(--card)] p-4"><p className="text-xs text-[color:var(--muted-fg)]">{label}</p><p className={cn("text-xl font-semibold", warn && "text-warn-fg")}>{value}</p>{sub && <p className="text-xs text-[color:var(--muted-fg)]">{sub}</p>}</div>;
}
...
function InvoiceLinesTable({ form, updateLine, removeLine }: { form: CreateArSalesInvoiceDto; updateLine: (index: number, patch: Partial<CreateArSalesInvoiceDto["lines"][number]>) => void; removeLine: (index: number) => void }) {
  return <DrawerSection title="Dòng hàng hóa / dịch vụ"><div className="overflow-x-auto"><table className="min-w-full text-xs"><thead className="text-left text-[color:var(--muted-fg)]"><tr><th className="pb-1 pr-2">Diễn giải</th><th className="pb-1 pr-2 text-right w-16">SL</th><th className="pb-1 pr-2 text-right w-28">Đơn giá</th><th className="pb-1 pr-2 text-right w-16">VAT %</th><th className="pb-1 text-right w-24">Thành tiền</th><th className="pb-1 w-8" /></tr></thead><tbody>{form.lines.map((line, i) => <tr key={i} className="border-t border-[color:var(--border)]"><td className="py-1 pr-2"><input className={inputCls} value={line.description} onChange={(e) => updateLine(i, { description: e.target.value })} /></td><td className="py-1 pr-2"><input className={cn(inputCls, "text-right")} type="number" min="0" value={line.quantity} onChange={(e) => updateLine(i, { quantity: Number(e.target.value) })} /></td><td className="py-1 pr-2"><input className={cn(inputCls, "text-right")} type="number" min="0" value={line.unit_price} onChange={(e) => updateLine(i, { unit_price: Number(e.target.value) })} /></td><td className="py-1 pr-2"><input className={cn(inputCls, "text-right")} type="number" min="0" max="100" value={line.tax_rate} onChange={(e) => updateLine(i, { tax_rate: Number(e.target.value) })} /></td><td className="py-1 text-right font-semibold">{money(line.quantity * line.unit_price * (1 + (line.tax_rate ?? 0) / 100))}</td><td className="py-1 pl-2"><button onClick={() => removeLine(i)} className="text-[color:var(--muted-fg)] hover:text-red-500">✕</button></td></tr>)}</tbody></table></div></DrawerSection>;
}
