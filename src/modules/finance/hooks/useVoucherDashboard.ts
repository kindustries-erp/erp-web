...
export function useVoucherDashboard() {
  // KPI
  const [summary, setSummary] = useState<{ receipt: number; payment: number } | null>(null);
  ...
  const [chartLabels, setChartLabels] = useState<string[]>(["T1", "T2", "T3", "T4", "T5", "T6"]);
  ...
  async function loadSummary(
    ...
    try {
      const res = await getPaymentVouchersSummaryApi({
        ...
        status: "POSTED",
        ...(params.channelFilter ? { [params.channelParam]: params.channelFilter } : {}),
  ...
  async function loadOpeningBalanceAndChart(
    ...
    try {
      ...
      const [obRes, summaries] = await Promise.all([
        ...
        Promise.all(
          months.map((mo) =>
            ...
            }).catch(() => ({ total_receipt: 0, total_payment: 0, net: 0, total_count: 0, breakdown: [] })),
          ),
        ),
      ]);
      ...
      for (const b of obRes.items) {
        const hasChannel =
          params.voucherChannel === "CASH" ? !!b.cash_fund_id : !!b.company_bank_account_id;
        if (!hasChannel) continue;
        const channelId =
          params.voucherChannel === "CASH" ? b.cash_fund_id : b.company_bank_account_id;
        if (params.channelFilter && channelId !== params.channelFilter) continue;
        obByPeriod[b.fiscal_period] =
          (obByPeriod[b.fiscal_period] ?? 0) + Number(b.debit_amount ?? 0) - Number(b.credit_amount ?? 0);
      }

      const anchorPeriod =
        Object.keys(obByPeriod).filter((p) => p <= months[5].fiscalPeriod).sort().pop() ?? null;
      const anchorOb = anchorPeriod ? (obByPeriod[anchorPeriod] ?? 0) : 0;
      const anchorIdx = anchorPeriod ? months.findIndex((mo) => mo.fiscalPeriod === anchorPeriod) : -1;

      const endBals = new Array<number>(6).fill(0);
      if (anchorIdx >= 0) {
        endBals[anchorIdx] = anchorOb + summaries[anchorIdx].total_receipt - summaries[anchorIdx].total_payment;
        for (let i = anchorIdx + 1; i < 6; i++) {
          endBals[i] = endBals[i - 1] + summaries[i].total_receipt - summaries[i].total_payment;
        }
        if (anchorIdx > 0) {
          endBals[anchorIdx - 1] = anchorOb;
          for (let i = anchorIdx - 2; i >= 0; i--) {
            endBals[i] = endBals[i + 1] - summaries[i + 1].total_receipt + summaries[i + 1].total_payment;
          }
        }
      } else {
        let bal = anchorOb;
        if (anchorPeriod) {
          ...
          if (anchorStart <= beforeChartStart) {
            ...
            bal += (preChartSummary.total_receipt ?? 0) - (preChartSummary.total_payment ?? 0);
          }
        }
        for (let i = 0; i < 6; i++) {
          endBals[i] = bal + summaries[i].total_receipt - summaries[i].total_payment;
          bal = endBals[i];
        }
      }
      ...
      if (from) {
        ...
        const openingAnchorPeriod =
          Object.keys(obByPeriod).filter((p) => p <= fromPeriod).sort().pop() ?? null;
        let filterOpeningBal = openingAnchorPeriod ? (obByPeriod[openingAnchorPeriod] ?? 0) : 0;
        if (openingAnchorPeriod) {
          ...
          if (anchorStart <= beforeFrom) {
            ...
            filterOpeningBal +=
              (preFilterSummary.total_receipt ?? 0) - (preFilterSummary.total_payment ?? 0);
          }
        }
      ...
      const data = endBals.map((b, i) => {
        const hasMonthData =
          summaries[i].total_count > 0 || obByPeriod[months[i].fiscalPeriod] != null;
  ...
  async function loadDonutData(
    ...
    try {
      ...
      const items =
        channelFilter
          ? res.items.filter((v) => (v as unknown as Record<string, unknown>)[params.channelParam] === channelFilter)
          : res.items;
